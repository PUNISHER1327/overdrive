...
const GalleryPage = ({ startIndex, images, onImageClick }) => {
  ...
  // Wrap modulus array access to endlessly loop images if startIndex is high
  const pageImages = Array.from({ length: 8 }).map((_, i) => images[(startIndex + i) % images.length]);
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-6 auto-rows-[200px] md:auto-rows-[250px] w-full">
        {pageImages.map((img, index) => (
          <div
            ...
            className={`relative overflow-hidden rounded-[30px] border border-[#1A1A1A] group cursor-pointer ${img.size}`}
          >
            <div className="absolute inset-0 group-hover:scale-105 transition-transform duration-700">
              <img 
                src={img.url} 
                className="w-full h-full object-cover opacity-60 mix-blend-luminosity brightness-75 group-hover:opacity-80 group-hover:mix-blend-normal transition-all duration-700 pointer-events-none" 
              />
    ...
    const scroll = () => {
      if (scrollRef.current && !isHovered && !selectedImg) {
         ...
         if (scrollRef.current.scrollLeft >= scrollRef.current.scrollWidth / 2) {
             scrollRef.current.scrollLeft = 0; // Jump back stealthily
         }
      }
  ...
  return (
    <section id="gallery" className="bg-[#0A0A0A] py-32 overflow-hidden border-b border-[#1F1F1F]">
      <div className="container mx-auto px-6">
        <div className="flex flex-col md:flex-row justify-between items-end mb-20 gap-8">
          ...
          <div className="flex gap-4">
             <button onClick={scrollLeft} className="w-14 h-14 rounded-full border border-white/20 flex items-center justify-center text-white hover:bg-white hover:text-black transition-colors shrink-0">
               <ArrowLeft size={24}/>
             </button>
             <button onClick={scrollRight} className="w-14 h-14 rounded-full border border-white/20 flex items-center justify-center text-white hover:bg-white hover:text-black transition-colors shrink-0">
               <ArrowRight size={24}/>
             </button>
          </div>
        </div>
      ...
      >
         ...
         {Array.from({ length: Math.ceil(images.length / 8) }).map((_, chunkIndex) => (
            <GalleryPage key={`orig-${chunkIndex}`} startIndex={chunkIndex * 8} images={images} onImageClick={setSelectedImg} />
         ))}
         {/* Duplicated Chunked Pages for infinite scroll illusion */}
         {Array.from({ length: Math.ceil(images.length / 8) }).map((_, chunkIndex) => (
            <GalleryPage key={`dup-${chunkIndex}`} startIndex={chunkIndex * 8} images={images} onImageClick={setSelectedImg} />
         ))}
      </div>
